...
export const supabasePromoterRepository: PromoterRepository = {
  ...
  async applyByToken({ token, applicantId, message, name }) {
    const db = supabaseAdmin();
    const resolved = await this.resolveInviteToken(token);
    if (!resolved) return err("invalid_token");

    // El organizador ve applicant.full_name en la lista de solicitudes
    // (listPendingApplications) — si el postulante editó su nombre en el
    // formulario, persistirlo, si no queda descartado silenciosamente (LOW-11).
