 ...
 * acción lo lleva a login y regresa a esta misma página (?next). Si está
 * logueado, hace toggle optimista.
 */
export function FollowButton({
  ...
  const onClick = () => {
    if (!isLogged) {
      router.push(`/login?next=/${orgSlug}` as never);
      return;
    }
    toggle();
  };
  ...
  if (isLogged && isFollowing) {
    return (
      <button
        ...
        disabled={isPending}
        className={
          "group inline-flex items-center gap-1.5 rounded-full border border-cart-line bg-cart-bg-elev font-semibold text-white transition hover:border-rose-400/50 hover:text-rose-200 disabled:opacity-60 " +
          cls
        }
      >
