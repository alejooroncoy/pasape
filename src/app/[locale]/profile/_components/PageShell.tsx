...
export function BackLink({ href = "/profile", label = "Perfil" }: { href?: string; label?: string }) {
  return (
    <Link
      href={href as never}
      className="inline-flex items-center gap-1.5 py-2 text-[13px] font-medium text-white/55 transition hover:text-white"
    >
...
export function PageTitle({ title, subtitle }: { title: string; subtitle?: string }) {
  return (
    <header className="pb-1 pt-2">
      ...
      {subtitle && <p className="mt-1.5 text-[13.5px] text-white/55">{subtitle}</p>}
    </header>
  );
}
