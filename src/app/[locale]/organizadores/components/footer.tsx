...
export function SiteFooter() {
  return (
    <footer className="relative z-[1] border-t border-cart-line bg-cart-bg px-0 py-12 pb-15">
      <div className="mx-auto w-full max-w-[1160px] px-[22px] md:px-8">
        <div className="grid gap-10 md:grid-cols-[1.4fr_repeat(3,minmax(0,1fr))]">
          ...
          />
          <div>
            <h5 className="m-0 mb-3.5 text-[11.5px] font-semibold uppercase tracking-[0.1em] text-cart-ink-2">
...
function FootCol({ title, links }: { title: string; links: Array<[string, string]> }) {
  return (
    <div>
      ...
      <ul className="m-0 flex list-none flex-col gap-2 p-0 text-sm text-cart-ink-3">
        {links.map(([label, href]) => (
          <li key={label}>
            <Link href={href} className="transition-colors hover:text-cart-ink">
              {label}
            </Link>
          </li>
        ))}
